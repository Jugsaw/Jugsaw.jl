...
function tree2adt(t){
    if (t.data == "expr") {
        return tree2adt(t.children[0]);
    } else if (t.data == "number" || t.data == "string") {
        return JSON.parse(t.children[0].value);
    } else if (t.data == "true") {
        return true;
        ...
        return t.children.map(tree2adt);
    } else if (t.data == "call") {
        return new Call(tree2adt(t.children[0]), tree2adt(t.children[1]), tree2adt(t.children[2]))
    } else if (t.data == "untyped") {
        return {"type" : "unspecified", "fields" : t.children.map(tree2adt)};
    } else if (t.data == "object") {
        const [typename, ...fieldvalues] = t.children
        return {"type" : tree2adt(typename), "fields" : fieldvalues.map(tree2adt)};
    } else {
        throw new Error(`Grammar Error: syntax ${t.data} is not defined!`);
    }
}
