...
@testset "Julia code" begin
    ...
    code = generate_code("Julia", "jugsaw.co", :testapp, :f, 1, adt, typetable)
    @test code == """using Jugsaw.Client
app = request_app(ClientContext(; endpoint="jugsaw.co"), :testapp)
lazyreturn = app.f[1](nothing, true, 1, 1.0, [1, 2, 3], reshape([1, 3, 2, 4], 2, 2), Dict(2 => 3), "EA", (1, 2); a = 1.0e-8, b = Dict(2 => 4))
...
@testset "Python code" begin
    ...
    democall = JugsawIR.Call(:f, (nothing, true, 1, 1.0, [1, 2, 3], [1 2; 3 4], Dict(2=>3), EA, 1+2im), (; a=1e-8, b=Dict(2=>4)))
    adt, typetable = JugsawIR.julia2adt(democall)
    code = generate_code("Python", "jugsaw.co", :testapp, :f, 1, adt, typetable)
    @test code == """import jugsaw, numpy
app = jugsaw.request_app(jugsaw.ClientContext(endpoint="jugsaw.co"), "testapp")
lazyreturn = app.f[0](None, True, 1, 1.0, [1, 2, 3], numpy.reshape([1, 3, 2, 4], (2, 2), order='F'), {2:3}, "EA", (1, 2), a = 1.0e-8, b = {2:4})
result = lazyreturn()   # fetch result"""
end
...
@testset "Javascript code" begin
    ...
    democall = JugsawIR.Call(:f, (nothing, true, 1, 1.0, [1, 2, 3], [1 2; 3 4], Dict(2=>3), EA, 1+2im), (; a=1e-8, b=Dict(2=>4)))
    adt, typetable = JugsawIR.julia2adt(democall)
    code = generate_code("Javascript", "jugsaw.co", :testapp, :f, 1, adt, typetable)
...
app_promise.then(app=>app.call("f", 0, [null, true, 1, 1.0, [[3], [1, 2, 3]], [[2, 2], [1, 3, 2, 4]], [[[1], [2]], [[1], [3]]], "EA", [1, 2]], [1.0e-8, [[[1], [2]], [[1], [4]]]])).then(console.log)
</script>"""
end